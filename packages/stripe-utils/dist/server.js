...
export class StripeHandler {
    ...
    webhook_key;
    stripe;
    constructor({ products, secret_key, webhook_key, dataTable }) {
    ...
    async createCheckoutSession({ mode = "subscription", productKey, successUrl, cancelUrl, customerEmail, supabaseId, customerId }) {
        try {
    ...
    async createCustomer({ email, supabaseId }) {
        try {
    ...
    async handleWebhook({ req, webhookConfig }) {
        const sig = req.headers.get("stripe-signature");
        if (!sig)
            throw new Error("No stripe-signature in headers from stripes webhook call!!!");
        const body = await req.text();
        const webhookSecret = this.webhook_key;
        if (!webhookSecret)
            throw new Error("No webhook_key");
        let event;
        try {
        ...
        switch (event.type) {
            case "checkout.session.completed":
                try {
                    ...
                    const c1_session = event.data.object;
            ...
            case "customer.subscription.created":
                try {
                    ...
                    const c5_subscription = event.data.object;
                    const c5_customer_id = c5_subscription.customer;
            ...
            case "customer.subscription.updated":
                try {
                    ...
                    const c6_subscription = event.data.object;
                    const c6_customer_id = c6_subscription.customer;
            ...
            case "invoice.payment_action_required":
                try {
                    ...
                    const c7_invoice = event.data.object;
                    const c7_customer_id = c7_invoice.customer;
            ...
            case "invoice.paid":
                try {
                    ...
                    const c2_invoice = event.data.object;
                    const c2_customer_id = c2_invoice.customer;
            ...
            case "invoice.payment_failed":
                try {
                    ...
                    const c3_invoice = event.data.object;
                    const c3_customer_id = c3_invoice.customer;
            ...
            case "customer.subscription.deleted":
                try {
                    ...
                    const c4_subscription = event.data.object;
                    const c4_customer_id = c4_subscription.customer;
    ...
    async createBillingPortal({ supabaseId, returnUrl }) {
        try {
            ...
            if (!stripeId_obj)
                throw new Error("No stripe id found for user");
    ...
    async updateUserAbo(userId, newStatus) {
        try {
            await this.dataTable.update({
                where: [{ column: "user_id", is: userId }],
                update: { stripe_subscription: { status: newStatus } }
            });
    ...
    async getSupabaseUserIdByStripeCustomerId(stripeCustomerId) {
        try {
            const users = await this.dataTable.select({
                columns: ["user_id"],
                where: [{ column: "stripe_id", is: stripeCustomerId }]
            });
    ...
    async getPriceID(identifier, type) {
        try {
            if (type === 'session') {
                // Von Session ID
                ...
                return invoice.lines.data[0]?.price?.id || undefined;
            }
        }
        catch (error) {
...
export function createStripeHandler(config) {
