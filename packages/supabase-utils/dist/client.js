 ...
 */
export async function sendSession({ supabase, toBackend, extraData }) {
    ...
    if (sessionError)
        throw new Error("fehler beim session Kriegen!");
    if (!data.session) {
        ...
        return {
            data: { message: "User not logged in", success: false },
            status: 401
        };
    }
    try {
        const { data: backendData, status } = await axios.post(toBackend, {
 ...
 */
export async function OAuthLogin({ supabase, provider = "google", scopes, redirectTo }) {
    ...
    if (SignInError)
        throw new Error("fehler beim OAuth Sign in!");
}
export async function addUser({ supabase, toTable }) {
    const { data: { user }, error: getUserError } = await supabase.auth.getUser();
    if (getUserError)
        throw new Error("Error beim user kriegen in der 'addUser' function, Error: " + getUserError);
    if (!user)
    ...
    if (user_id_obj)
        console.log("User mit der id: " + user.id + " ist bereits in der Tabelle: " + toTable.tableName + " vorhanden!");
    const user_obj = await toTable.insert([{ user_id: user.id }]);
    if (!user_obj)
        throw new Error("Error beim user in die Tabelle: " + toTable.tableName + " hinzufügen, Error: " + user_obj);
    return true;
}
